...
class StripeWH_Handler:
    ...
    def _send_confirmation_email(self, order, items):
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        for item_id, qty in json.loads(bag).items():
            ...
            name = product.name
            price = product.price
            items[item_no] = {'name': name, 'price': price, 'qty': qty, 'tot_cost': qty.items_by_size * price}
            item_no += 1
        ...
        while attempt <= 5:
            try:
                ...
                time.sleep(1)
        if order_exists:
            self._send_confirmation_email(order, items)
            return HttpResponse(
